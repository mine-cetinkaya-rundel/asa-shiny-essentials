...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      actionButton(inputId = "write_csv",              # button to <------------
                   label = "Write CSV"),               # write CSV <------------
...
server <- function(input, output) {
  ...
  observeEvent(                                      
    input$write_csv, {                             # when button is clicked <---
      ...
      filename <- glue("movies_sample_{now}.csv")  # create filename <----------
      
      write_csv(movies_sample(), path = filename)  # write to csv <-------------
